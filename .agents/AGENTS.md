...
_This checklist must be executed after generating every new article and before saving or modifying any `.mdx` file. An article is not considered complete until every item passes._
...
# LEOS FINALIZATION ORDER (v2.0)

## CORE PHILOSOPHY

LEOS exists to publish exceptional content—not to become an endlessly evolving software project. Every future change to LEOS must solve a demonstrated production problem. Avoid feature creep. Prefer stability over novelty. Editorial quality always takes precedence over automation.

## DIRECTIVE 1: READ-ONLY BY DEFAULT

Every audit stage shall execute in READ-ONLY mode unless explicitly instructed otherwise. The default behavior is Analyze, Validate, Report, Recommend. Never modify project files automatically during an audit.

## DIRECTIVE 2: EXPLICIT APPLY MODE

Safe modifications may only occur after explicit editorial approval. When approved, LEOS may execute an APPLY MODE for low-risk maintenance tasks (Frontmatter completion, Content Intelligence sync, Internal link repair, Image path/formatting/whitespace normalization). High-risk operations remain prohibited without separate approval.

## DIRECTIVE 3: CONFIDENCE-BASED METADATA

Every automatically generated metadata field shall receive a confidence classification:

- **HIGH**: canonical, description from summary, draft, lastUpdated, featuredImage path
- **MEDIUM**: imageAlt, keywords, categories
- **LOW**: semantic keyword inference, audience inference, search intent inference, monetization inference

Only HIGH confidence fields may be automatically written during APPLY MODE. MEDIUM and LOW confidence suggestions must be reported for editorial review unless explicitly approved.

## DIRECTIVE 4: FRONTMATTER VERSIONING

Every article shall include a schema version (e.g. `frontmatterVersion: '2.0'`). This enables future schema upgrades without manual inspection.

## DIRECTIVE 5: AUTOMATED CHANGELOG

Every APPLY MODE execution must generate `LEOS_CHANGELOG.md` containing: Date, LEOS Version, Files Modified, Fields Added, Fields Updated, Safe Fixes Applied, Skipped Items, Manual Review Required. No automated modification may occur without being recorded.

## DIRECTIVE 6: THREE-PASS RELEASE SYSTEM

The Release Gate shall permanently operate using three phases:

- **PHASE A (Audit)**: Read-only, Generate Release Report.
- **PHASE B (Safe Patch)**: Apply only approved SAFE fixes, Generate Patch Report.
- **PHASE C (Verification)**: Re-run the complete audit. Publication is approved only if verification passes.

## DIRECTIVE 7: LEOS CHANGE POLICY

Future LEOS improvements are permitted only if they satisfy at least one of the following: Fix a verified production issue, Reduce editorial workload, Improve technical correctness, Improve SEO quality, Improve maintainability, Improve publication safety. New features shall not be added solely because they are technically possible.

## DIRECTIVE 8: STABILITY FIRST

After this order, LEOS shall be considered operationally frozen. Minor maintenance updates are allowed. Major architectural redesigns require explicit approval from the Editorial Board.

## DIRECTIVE 9: EDITORIAL AUTHORITY

Automation assists editors. Automation never replaces editorial judgment. When automated output conflicts with human editorial expertise, the editorial decision always prevails.

## DIRECTIVE 10: LONG-TERM OBJECTIVE

The primary mission is enabling Locitra to publish Platinum-quality content consistently across hundreds of articles and multiple commercial clusters. Future success will be measured by Publication quality, Reader value, Technical excellence, Sustainable topical authority, Commercial performance.

## DIRECTIVE 11: WORKFLOW ORCHESTRATOR SINGLE RESPONSIBILITY

The Workflow Orchestrator is NOT an editorial, research, writing, SEO, or publishing agent. It is a workflow controller only.

1. **Single Responsibility**: Each LEOS component owns exactly one responsibility. The Orchestrator's responsibility is workflow coordination.
2. **Artifact Ownership**: The Orchestrator owns NO editorial artifacts. It may only load, pass, track, archive, and summarize existing artifacts.
3. **State Management**: The Workflow Orchestrator is stateless. It must never permanently store editorial knowledge.
4. **Dependency Rule**: Lower-level agents must never know about higher-level agents. Only the Workflow Orchestrator coordinates execution.
5. **Permanence**: This architectural principle is permanently frozen. The Workflow Orchestrator must never absorb the responsibilities of specialist agents.

The operational behavior of LEOS is defined in `LEOS/MASTER_PROMPT.md`.
