...
const PROBLEMS: Array<{ id: DiagnosticProblem; label: string; description: string }> = [
  {
    id: 'indexing',
    label: "My pages aren't getting indexed",
    description: 'Check crawlability, indexability, canonicals, and sitemap signals.',
  },
  {
    id: 'traffic-drop',
    label: 'My organic traffic dropped',
    description: 'Start with technical signals that may coincide with a traffic change.',
  },
  {
    id: 'wrong-page',
    label: 'Google is showing the wrong page',
    description: 'Investigate canonical and duplicate URL signals.',
  },
  {
    id: 'slow',
    label: 'My website is slow',
    description: 'Start with images, markup, resources, and performance signals.',
  },
  {
    id: 'technical',
    label: 'I have technical SEO errors',
    description: 'Run the broad technical quick check.',
  },
  {
    id: 'schema',
    label: 'My schema / structured data has problems',
    description: 'Inspect JSON-LD presence and parsing.',
  },
  {
    id: 'migration',
    label: 'I recently redesigned or migrated my website',
    description: 'Look for common migration and URL consistency risks.',
  },
  {
    id: 'broken-links',
    label: 'I have broken pages or links',
    description: 'Inspect response, redirects, and internal linking signals.',
  },
  {
    id: 'duplicates',
    label: 'I have duplicate or low-value pages',
    description: 'Start with canonical and architecture signals.',
  },
  {
    id: 'unknown',
    label: "I don't know — find the important problems",
    description: 'Use the general technical troubleshooting path.',
  },
]
...
function severityClass(severity: CrawlReportResult['findings'][number]['severity']) {
  switch (severity) {
    case 'critical':
...
export default function TechnicalSEOTroubleshooter() {
  ...
  const [result, setResult] = useState<CrawlReportResult | null>(null)
  ...
  async function pollScan(scanId: string) {
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
      ...
      if (!response.ok) {
        throw new Error(data.errorMessage || 'Unable to read the Technical SEO scan status.')
      }
      ...
      if (data.status === 'failed' || data.status === 'cancelled') {
        throw new Error(data.errorMessage || 'The Technical SEO scan did not complete.')
      }
    ...
    <div className="mx-auto max-w-6xl">
      <section className="rounded-3xl border border-gray-200 bg-white px-6 py-12 shadow-sm sm:px-10 dark:border-gray-800 dark:bg-gray-950">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <p className="mx-auto mt-5 max-w-2xl text-lg leading-8 text-gray-600 dark:text-gray-400">
            Enter your website, tell us what you are worried about, and get an evidence-based first
            diagnostic instead of a dump of generic SEO errors.
          </p>
        </div>
        ...
        <form onSubmit={handleAnalyze} className="mx-auto mt-10 max-w-4xl space-y-8">
          <div>
            ...
            <input
              ...
              className="focus:border-primary-500 focus:ring-primary-200 w-full rounded-xl border border-gray-300 bg-white px-4 py-3 text-base text-gray-900 transition outline-none focus:ring-2 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100"
              required
            />
          </div>
                ...
                return (
                  <button
                    ...
                    className={`rounded-2xl border p-4 text-left transition ${
                      active
                        ? 'border-primary-500 bg-primary-50 dark:border-primary-400 dark:bg-primary-950/30 shadow-sm'
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      ...
                      {!item.enabled && (
                        <span className="rounded-full bg-gray-200 px-2 py-0.5 text-[10px] font-semibold tracking-wide text-gray-600 uppercase dark:bg-gray-800 dark:text-gray-300">
                          Next
                        </span>
                      )}
                    </div>
      ...
      {result &&
        (() => {
          ...
          const renderFinding = (finding: CrawlReportResult['findings'][number]) => (
            <article
              key={finding.id}
            ...
            >
              ...
              <div className="mt-4">
                <p className="text-xs font-bold tracking-wide uppercase opacity-80">Evidence</p>
          ...
          return (
            <section className="mt-8 space-y-6">
                ...
                <div className="flex flex-col justify-between gap-6 md:flex-row md:items-end">
                  <div>
                    ...
                    {result.diagnosticFocus && (
                      <div className="border-primary-200 bg-primary-50 text-primary-800 dark:border-primary-900 dark:bg-primary-950/30 dark:text-primary-200 mt-3 inline-flex max-w-full items-center gap-2 rounded-full border px-3 py-1.5 text-xs font-semibold">
                        <span>Diagnostic focus</span>
                        <span aria-hidden="true">·</span>
                        <span className="truncate">{result.diagnosticFocus.label}</span>
                      </div>
                    )}
                    <h2 className="mt-3 text-2xl font-extrabold break-all text-gray-900 dark:text-gray-100">
                      {result.finalUrl}
                    </h2>
                  ...
                  <div className="grid grid-cols-5 gap-2 text-center">
                    {(
                      ...
                      <div
                        key={key}
                        className="min-w-14 rounded-xl border border-gray-200 px-2 py-3 dark:border-gray-800"
                      >
              ...
              <div className="border-primary-200 bg-primary-50 dark:border-primary-900 dark:bg-primary-950/20 rounded-3xl border p-6 shadow-sm">
                <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
                  <div>
                    ...
                    <h3 className="mt-2 text-2xl font-extrabold text-gray-900 dark:text-gray-100">
                      {result.pagesChecked} {result.pagesChecked === 1 ? 'page' : 'pages'} checked.
                      Want us to investigate more?
                    </h3>
                    <p className="mt-3 max-w-2xl text-sm leading-6 text-gray-600 dark:text-gray-400">
                      This free check crawled up to the first 5 same-site HTML pages it could
                      discover from sitemap and internal-link signals. It found{' '}
                      {result.urlsDiscovered} URL{result.urlsDiscovered === 1 ? '' : 's'} in the
                      crawl queue; {result.urlsNotCrawled} remain outside this limited scan.
                      {result.urlsBlockedByRobots > 0 && (
                        <>
                          {' '}
                          {result.urlsBlockedByRobots} discovered URL
                          {result.urlsBlockedByRobots === 1 ? '' : 's'} were skipped because of
                          robots.txt rules.
                        </>
                      )}
                    </p>
                  ...
                  <div className="border-primary-200 dark:border-primary-900 rounded-2xl border bg-white p-5 dark:bg-gray-950">
                  ...
                  {!isGeneral && focusedFindings.length === 0 && (
                    <section>
                      ...
                      <div className="mt-4 rounded-2xl border border-emerald-200 bg-emerald-50 p-5 text-sm leading-6 text-emerald-900 dark:border-emerald-900 dark:bg-emerald-950/30 dark:text-emerald-200">
                        <strong>
                          {focusId === 'indexing' &&
                            'No obvious indexing blocker was found in this quick check.'}
                          {focusId === 'schema' &&
                            'No schema syntax problem was found in this quick check.'}
                          {focusId === 'wrong-page' &&
                            'No obvious canonical or duplicate signal matched this quick check.'}
                          {focusId === 'slow' &&
                            'No performance-related finding matched the checks available in this quick scan.'}
                          {focusId === 'broken-links' &&
                            'No broken-link finding was detected by this quick scan.'}
                          {focusId === 'migration' &&
                            'No migration-related technical finding was detected by this quick scan.'}
                          {focusId === 'duplicates' &&
                            'No duplicate or low-value-page finding was detected by this quick scan.'}
                          {focusId === 'technical' &&
                            'No confirmed technical problem was detected by this quick scan.'}
                          {focusId === 'traffic-drop' &&
                            'No technical finding associated with a traffic drop was detected by this quick scan.'}
                        </strong>
                        <p className="mt-2">
                          This is a limited initial-HTML check. A deeper crawl or additional data
                          may still be required to investigate the selected concern. For indexing
                          and traffic investigations, Google Search Console will be important in a
                          deeper plan.
                        </p>
                      </div>
                    </section>
                  ...
                  {!isGeneral && otherFindings.length > 0 && (
                    <details className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-gray-950">
                      ...
                      <p className="mt-3 text-sm leading-6 text-gray-500 dark:text-gray-400">
                        These findings were detected by the quick scan but are not directly
                        associated with your selected troubleshooting concern.
                      </p>
                      <div className="mt-4 space-y-4">{otherFindings.map(renderFinding)}</div>
                    </details>
                  )}
                </div>
                ...
                <aside className="h-fit rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-gray-950">
                  ...
                  <div className="mt-4 space-y-3 text-sm text-gray-600 dark:text-gray-400">
                    ...
                    <div className="flex justify-between gap-4">
                      ...
                      <strong className="text-gray-900 dark:text-gray-100">
                        {result.urlsBlockedByRobots}
                      </strong>
                    </div>
                    <div className="flex justify-between gap-4">
                      <span>Internal links</span>
                      <strong className="text-gray-900 dark:text-gray-100">
                        {result.metrics.internalLinks}
                      </strong>
                    </div>
                    <div className="flex justify-between gap-4">
                      <span>External links</span>
                      <strong className="text-gray-900 dark:text-gray-100">
                        {result.metrics.externalLinks}
                      </strong>
                    </div>
                    <div className="flex justify-between gap-4">
                      <span>Images</span>
                      <strong className="text-gray-900 dark:text-gray-100">
                        {result.metrics.images}
                      </strong>
                    </div>
                    <div className="flex justify-between gap-4">
                      <span>Missing alt attributes</span>
                      <strong className="text-gray-900 dark:text-gray-100">
                        {result.metrics.imagesWithoutAlt}
                      </strong>
                    </div>
                    <div className="flex justify-between gap-4">
                      <span>Empty alt attributes</span>
                      <strong className="text-gray-900 dark:text-gray-100">
                        {result.metrics.imagesWithEmptyAlt}
                      </strong>
                    </div>
                    <div className="flex justify-between gap-4">
                    ...
                    <div className="flex justify-between gap-4">
                      ...
                      <strong className="text-gray-900 dark:text-gray-100">
                        {result.metrics.jsonLdBlocks}
                      </strong>
                    </div>
                    <div className="flex justify-between gap-4">
                      <span>hreflang links</span>
                      <strong className="text-gray-900 dark:text-gray-100">
                  ...
                  <div className="mt-6 rounded-xl bg-gray-50 p-4 text-xs leading-5 text-gray-600 dark:bg-gray-900 dark:text-gray-400">
                    ...
                    {result.diagnosticFocus && (
                      <span className="mt-2 block font-medium">
                        ...
                        {result.diagnosticFocus.matchedFindings}
                      </span>
                    )}
                  </div>
                </aside>
