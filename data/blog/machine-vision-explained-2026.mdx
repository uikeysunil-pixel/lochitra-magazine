...
- **More Than Just Cameras:** A camera only captures light. Machine vision relies on Artificial Intelligence (specifically "Deep Learning" neural networks) to actually _understand_ the context of the light the camera captures.
...
Simply put: Computer Vision is the _software_ that understands the image. Machine Vision is the _complete system_ (software + hardware) that uses that understanding to do physical work.
